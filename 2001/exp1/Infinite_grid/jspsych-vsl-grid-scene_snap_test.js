...
var jsPsychVslGridSceneSnapTest = (function (jspsych) {
  ...
  const info = {
    ...
    parameters: {
      ...
      grid_shift: {
        type: jspsych.ParameterType.INT,
        pretty_name: 'grid shift',
        default: 0,
        description: 'The left (-3) or right (3) shift of the test grid'
      },
      grid_size: {
        type: jspsych.ParameterType.INT,
  ...
  class VslGridSceneSnapPlugin {
    ...
    trial(display_element, trial) {
      ...
      var stimuli_start_col = trial.stim_scene_start[1] + trial.grid_shift;
      ...
      for (var row = 0; row < nrows_grid; row++) {
        for (var col = 0; col < ncols_grid; col++) {
          var cell = paper.rect(trial.image_size[0] * (col), trial.image_size[1] * (row), trial.image_size[0], trial.image_size[1]).attr({
            "fill-opacity": 0,
            stroke: 'black',
      ...
      var outline = paper.rect(0, 0, trial.image_size[0] * nrows_grid, trial.image_size[1] * ncols_grid).attr({
        "fill-opacity": 0,
        stroke: 'black',
